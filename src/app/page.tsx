...
function getDirectoryTone(_brandPack: string) {
  return {
    ...
    hero: 'bg-secondary/30',
    panel: 'border border-border bg-card shadow-[0_24px_64px_rgba(15,23,42,0.08)]',
    soft: 'border border-border bg-secondary/40',
    muted: 'text-muted-foreground',
    title: 'text-foreground',
    badge: 'bg-primary text-primary-foreground',
    action: 'bg-primary text-primary-foreground hover:opacity-90',
    actionAlt: 'border border-border bg-card text-foreground hover:bg-secondary/50',
  }
}
...
function getEditorialTone() {
  return {
    shell: 'bg-background text-foreground',
    panel: 'border border-border bg-card shadow-[0_24px_60px_rgba(15,23,42,0.08)]',
    soft: 'border border-border bg-secondary/40',
    muted: 'text-muted-foreground',
    title: 'text-foreground',
    badge: 'bg-primary text-primary-foreground',
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {classifiedPosts.slice(0, 4).map((post) => {
              ...
              return (
                <Link key={post.id} href={getTaskHref('classified', post.slug)} className={`overflow-hidden rounded-[1.8rem] ${tone.panel}`}>
                  <div className="relative h-44 overflow-hidden">
                    <ContentImage src={getPostImage(post)} alt={post.title} fill className="object-cover" />
                  </div>
                  <div className="p-5">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.24em] opacity-70">{meta.category || 'Classified'}</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-18">
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <div>
            ...
            <h1 className={`mt-6 max-w-4xl text-5xl font-semibold tracking-[-0.06em] sm:text-6xl ${tone.title}`}>
              A cleaner, more immersive way to browse and explore.
            </h1>
            <p className={`mt-6 max-w-2xl text-base leading-8 ${tone.muted}`}>{SITE_CONFIG.description}</p>
            <div className="mt-8 flex flex-wrap gap-3">
